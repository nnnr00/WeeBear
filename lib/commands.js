...
bot.action('complete_upload', async (ctx) => {
  ...
  // 清空缓冲区
  await db.query(`TRUNCATE TABLE p_buffer`);
...
bot.command('dh', async (ctx) => {
  ...
  // 根据数据库类型构建分页查询
  let query = `SELECT * FROM p_buffer ORDER BY created_at ASC LIMIT ${limit} OFFSET ${offset}`;
  if (DB_TYPE === 'pg') {
    query = `SELECT * FROM p_buffer ORDER BY created_at ASC LIMIT ${limit} OFFSET ${offset}`;
  }
  ...
  if (keywords.length === 0) {
    ...
    // 构建分页消息
    const totalPages = Math.ceil((await db.query('SELECT COUNT(*) AS total FROM p_buffer'))[0].total / limit);
...
bot.action(/^dh_page_(\d+)$/, async (ctx) => {
  ...
  ctx.scene.leave(); // 退出当前场景
  await bot.handleCommand(ctx, '/dh'); // 重新进入 /dh
});
...
bot.command('tickets', async (ctx) => {
  ...
  const totalPages = Math.ceil((await db.query(
  ...
  ctx.replyWithMarkdownV2(
    `📄 *工单列表 (1/${totalPages})* 📄\n\n` +
    ticketList +
    `\n\n🗑️ [删除工单](${FILE_ID_TABLE.COLUMNS.ORDER_ID})`,
    Markup.inlineKeyboard([
...
bot.command('users', async (ctx) => {
  ...
  const [users] = await db.query(
    `SELECT * FROM file_ids ORDER BY created_at DESC LIMIT 10`
  );
